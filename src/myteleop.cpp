...
private:
  ...
  void goalResponseCallback(std::shared_future<rclcpp_action::ClientGoalHandle<my_test::action::RotateAbsolute>::SharedPtr> future);
  void cancelGoal();
  
  rclcpp::Node::SharedPtr nh_;
  double linear_, angular_, l_scale_, a_scale_;
  rclcpp::Publisher<geometry_msgs::msg::Twist>::SharedPtr twist_pub_;
  rclcpp_action::Client<my_test::action::RotateAbsolute>::SharedPtr rotate_absolute_client_;
  rclcpp_action::ClientGoalHandle<my_test::action::RotateAbsolute>::SharedPtr goal_handle_;
};
...
MyTeleop::MyTeleop():
  ...
  a_scale_(2.0)
{
  nh_ = rclcpp::Node::make_shared("my_teleop");
  ...
  rotate_absolute_client_ = rclcpp_action::create_client<my_test::action::RotateAbsolute>(nh_, "turtle1/rotate_absolute");
}

void MyTeleop::sendGoal(float theta)
{
  auto goal = my_test::action::RotateAbsolute::Goal();
  goal.theta = theta;
  auto send_goal_options = rclcpp_action::Client<my_test::action::RotateAbsolute>::SendGoalOptions();
  send_goal_options.goal_response_callback =
    [this](std::shared_future<rclcpp_action::ClientGoalHandle<my_test::action::RotateAbsolute>::SharedPtr> future)
    {
...
void MyTeleop::goalResponseCallback(std::shared_future<rclcpp_action::ClientGoalHandle<my_test::action::RotateAbsolute>::SharedPtr> future)
{
  RCLCPP_DEBUG(nh_->get_logger(), "Goal response received");
  this->goal_handle_ = future.get();
}

void MyTeleop::cancelGoal()
{
 if (goal_handle_)
 {
...
int MyTeleop::keyLoop()
{
  char c;
  bool dirty=false;

  std::thread{std::bind(&MyTeleop::spin, this)}.detach();
